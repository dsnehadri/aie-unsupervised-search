...
// Instance-scaling vehicle rebuilt with the current object block: four head
// pre, four head post, two merge, projection, b1, b2, c1 and c = 15 tiles,
// against the 12 of the version this replaces. PRE_STREAM is deliberately
// OFF: it would have the PLIO drive a core stream directly, which hangs on
// hardware (it works only kernel-to-kernel, as in the chain graph).
class Obj20 : public adf::graph { public:
