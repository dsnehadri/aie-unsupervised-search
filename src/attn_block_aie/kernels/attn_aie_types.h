// attn_aie_types.h

#ifndef ATTN_AIE_TYPES_H
#define ATTN_AIE_TYPES_H

#include <aie_api/aie.hpp>
#include <adf.h>
    
// architectural constants 

constexpr int N_MAX = 12;
constexpr int E_DIM = 16;
constexpr int N_HEADS = 4;
constexpr int D_HEAD = 4;
constexpr int N_KV = 13;
constexpr int T_DIM = 3;
constexpr int T_KV = 4;

// ffn dimensions: 3-layer ffn after attention, all 16-wide

constexpr int FFN_NLAYERS = 3;
constexpr int FFN_DIM = E_DIM;

// fixed point configuration -- RETRAINED-weights scale layout (2026-08-10).
// Mirrors the PL datapath that delivers AUC 0.9639 with the retrained model:
//   data        Q6.9  (ap_fixed<16,7> bit-pattern)  range +/-64
//   weights     Q3.12 (ap_fixed<16,4> bit-pattern)  range +/-8   (all |w|<=1.1)
//   scores      Q10.5 (ap_fixed<16,11> bit-pattern) range +/-1024
// The wide +/-64 data range is REQUIRED: retrained candidate embeddings reach
// |17.9| (value_ranges.json), which overflows the old Q4.11. Scores at 5 frac
// mirror PL's widened score_t (cand Q*K^T reaches ~320). This is the config
// the old cand path already used; obj/cross now use it too (uniform).
// Bridge contract: PL data_t<16,7> bits == int16 at DATA_SCALE, PL
// score_t<16,11> bits (wij) == int16 at SCORE_SCALE. No AIE_FRAC11 gate.

constexpr int DATA_FRAC_BITS = 9;
constexpr int WEIGHT_FRAC_BITS = 12;
constexpr int ACC_SHIFT = WEIGHT_FRAC_BITS;      // data*weight >> 12 -> data scale
constexpr int AV_SHIFT = DATA_FRAC_BITS;         // attn(data)*V(data) >> 9 -> data scale

constexpr float DATA_SCALE = (float)(1 << DATA_FRAC_BITS);     // 512
constexpr float WEIGHT_SCALE = (float)(1 << WEIGHT_FRAC_BITS); // 4096

// Scores: obj/cross use Q8.7 (range +/-256; their retrained scores stay
// under ~90, and the finer 1/128 resolution avoids near-tie softmax flips
// the coarse 1/32 grid caused on ~30% of events). cand keeps Q10.5: its
// retrained Q*K^T reaches ~320 and needs the +/-1024 range.
constexpr int SCORE_FRAC_BITS = 7;
constexpr int SCORE_SHIFT = SCORE_FRAC_BITS;     // for scale_scores multiply
constexpr float SCORE_SCALE = (float)(1 << SCORE_FRAC_BITS);   // 128
// Q*Kt gemm shift: Q,K at DATA_SCALE; want result at SCORE_SCALE.
constexpr int QKT_SHIFT = DATA_FRAC_BITS + DATA_FRAC_BITS - SCORE_FRAC_BITS; // 11

constexpr int CAND_FRAC_BITS = DATA_FRAC_BITS;
constexpr int CAND_ACC_SHIFT = ACC_SHIFT;
constexpr float CAND_SCALE = DATA_SCALE;
constexpr int CAND_SCORE_FRAC_BITS = 5;
constexpr int CAND_SCORE_SHIFT = CAND_SCORE_FRAC_BITS;
constexpr float CAND_SCORE_SCALE = (float)(1 << CAND_SCORE_FRAC_BITS);  // 32
constexpr int CAND_QKT_SHIFT = DATA_FRAC_BITS + DATA_FRAC_BITS - CAND_SCORE_FRAC_BITS; // 13



// buffer sizes (in int16 elements)

// per head QKV projection weights: [E_DIM x D_HEAD] each
constexpr int QKV_WEIGHT_SIZE = E_DIM * D_HEAD; // 64 per matrix

// per head bias_kv, 2 x D_HEAD (one for K bias row, one for V bias row)
constexpr int BIAS_KV_SIZE = 2 * D_HEAD; // 8

// output projection weights: [E_DIM x E_DIM]
constexpr int OUT_PROJ_WEIGHT_SIZE = E_DIM * E_DIM; // 256

// attn input window: N_MAX x E_DIM
constexpr int ATTN_INPUT_SIZE = N_MAX * E_DIM; // 192

// per head intermediate buffers
constexpr int Q_SIZE = N_MAX * D_HEAD; // 48
constexpr int K_SIZE = N_KV * D_HEAD; // 52
constexpr int V_SIZE = N_KV * D_HEAD; // 52
constexpr int SCORE_SIZE = N_MAX * N_KV; // 156 (padding to 160 for alignment)
constexpr int HEAD_OUT_SIZE = N_MAX * D_HEAD; //48

// full attention output: N_MAX x E_DIM
constexpr int ATTN_OUTPUT_SIZE = N_MAX * E_DIM; // 192

// ffn weight sizes per layer
constexpr int FFN_WEIGHT_SIZE = E_DIM * E_DIM; // 256 per layer
constexpr int FFN_BIAS_SIZE = E_DIM; // 16 per layer

// layernorm params
constexpr int LN_PARAM_SIZE = E_DIM; // 16 (gamma) + 16 (beta)


// padding helpers
// aie mmul type is 4x4x4 for int16, dimensions should be multiples of 4
// n_max = 12, edim = 16, d_head = 4
// n_kv = 13, pad to 16 for k, v matrices

constexpr int N_KV_PAD = 16;

// window element type: int16 for the deployed build, float for the
// FLOAT_AIE unquantized x86sim reference (window sizes scale with it)
#ifdef FLOAT_AIE
typedef float aiedt;
#else
typedef int16 aiedt;
#endif

// wij_bias = [N_MAX x N_KV] per head
// in pl this was [N_HEADS x N_MAX x N_KV], in aie each head gets its own slice

constexpr int WIJ_SIZE = N_MAX * N_KV;

#endif