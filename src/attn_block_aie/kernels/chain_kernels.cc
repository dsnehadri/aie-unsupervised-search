                            ...
                            output_window_int16* __restrict x_out, bool zero_padded)
{
    ...
    // mask row: 1 = padded, lanes >= N_MAX zero
                    ...
                    output_window_int16* __restrict x_out, output_window_int16* __restrict c_out)
{
    ...
    for (int j = 0; j < N_MAX; j++) {
        ...
        // c[t] += row, wrap-around like the fabric's fixed-point add
        ...
        aie::store_v(c + t * E_DIM, aie::from_vector<acc80>(aie::add(a, b)).to_vector<int16>(0));
    }
