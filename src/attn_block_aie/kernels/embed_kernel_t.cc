               ...
               output_window_int16* __restrict embed_out)
{
    ...
    gemm_pk<E_DIM, EMBED_IN_PAD, 16>(W0_t.p, JT, HT, ACC_SHIFT);
    bias_rows<E_DIM>(HT, embed_b0);
    ln_lanes(HT, embed_ln0_g, embed_ln0_b, EPS_Q2);
    relu_rows<E_DIM>(HT);
    gemm_pk<E_DIM, E_DIM, 16>(W1_t.p, HT, GT, ACC_SHIFT);
    bias_rows<E_DIM>(GT, embed_b1);
    ln_lanes(GT, embed_ln1_g, embed_ln1_b, EPS_Q2);
    relu_rows<E_DIM>(GT);
    gemm_pk<E_DIM, E_DIM, 16>(W2_t.p, GT, HT, ACC_SHIFT);
    bias_rows<E_DIM>(HT, embed_b2);
